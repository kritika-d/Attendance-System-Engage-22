...
class Face:
    ...
    def check_face(self, file):
        image = face_recognition.load_image_file(self.load_train_file_by_name(file))
        try:
    ...
    def load_all(self):
        ...
        results = self.db.select('SELECT faces.id, faces.user_id, faces.filename, faces.created FROM faces ORDER BY faces.id')
